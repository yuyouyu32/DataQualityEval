...
def llm_eval_unit_test():
    ...
    for chunk in llm_eval(input_point, similar_bmgs):
        print(chunk, end='', flush=True)
        
def llm_regression_unit_test():
    # LLM Regression 分析测试
    input_point = {
        "Composition": "Al20(CoCrCuFeMnNiTiV)80",
        "Dmax(mm)": "13",
    ...
    for chunk in llm_regression(input_point, regression_results):
        print(chunk, end='', flush=True)


def llm_anomaly_unit_test():
    # LLM Anomaly 分析测试
    input_point = {
        "Composition": "Al20(CoCrCuFeMnNiTiV)80",
        "Dmax(mm)": "13",
    ...
    for chunk in llm_anomaly(input_point, anomaly_results):
        print(chunk, end='', flush=True)

def llm_outlier_unit_test():
    # LLM Outlier 分析测试
    input_point = {
        "Composition": "Al20(CoCrCuFeMnNiTiV)80",
        "Dmax(mm)": "13",
    ...
    for chunk in llm_outlier(input_point, outlier_results):
        print(chunk, end='', flush=True)
...
if __name__ == "__main__":
    # llm_eval_unit_test()
    # llm_regression_unit_test()
    # llm_anomaly_unit_test()
    llm_outlier_unit_test()